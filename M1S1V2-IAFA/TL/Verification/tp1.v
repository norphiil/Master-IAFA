...
Proof. 
  induction n.
  - intros. simpl. rewrite pow_1.
    lia.
  - intros. simpl.
    rewrite pow_add. rewrite IHn. rewrite mul_pow.
    reflexivity.
Qed.
...
Fixpoint merge {T} (l1 l2 : list T) : list T :=
  ...
  | x :: r1 =>
      match l2 with
      | [] => l1
      | y :: r2 => x::y::merge r1 r2
      end
  end.

Lemma merge_example : merge [1;2;3] [4;5] = [1;4;2;5;3].
Proof.
  reflexivity.
Qed.
...
(* Prove by induction on l1 the following property.
   ...
   and the tactic "lia" *)
Lemma merge_len : forall T (l1 l2 : list T),
